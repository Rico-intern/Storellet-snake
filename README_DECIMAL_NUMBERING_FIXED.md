...
3.5.10.12 Fix GAME-01 shield／invincibility collision position.
3.5.10.13 Fix GAME-02 monster knockback overlap.
3.5.10.14 Synchronise GAME-03 rules page and actual logic.
3.5.10.15 Confirm whether GAME-04 monsters may overlap food／treasure chests.
3.5.10.16 Define handling when the App enters the background.
3.5.10.17 Add daily attempt and point-deduction confirmation UI.
3.5.10.18 Add Exit and Session termination rules.

Phase 2: Before Production API UAT
3.5.10.19 Enforce the API and remove offline fallback when campaign-rule errors occur.
3.5.10.20 Complete Storellet identity authentication.
3.5.10.21 Complete production points checking and deduction.
3.5.10.22 Complete the Database and Session Lifecycle.
3.5.10.23 Fix the Idempotency implementation.
3.5.10.24 Add retry and reconciliation for failed submissions.
3.5.10.25 Make the frontend read Game Config in production.
3.5.10.26 Complete campaign start／end／disable controls.

Phase 3: Before Production
3.5.10.27 Complete coupon issuance and duplicate prevention.
3.5.10.28 Complete anti-cheat validation.
3.5.10.29 Restrict CORS and add Rate Limit and Security Headers.
3.5.10.30 Complete Audit Log, monitoring, and alerting.
3.5.10.31 Complete automated testing and iOS／Android physical-device UAT.
3.5.10.32 Complete “My Game Records” and customer-service reconciliation capabilities.
...
3.7.1 The API can be bypassed;
3.7.2 No production identity authentication;
3.7.3 No actual point deduction;
3.7.4 No production coupon issuance;
3.7.5 Scores are determined solely by the frontend;
3.7.6 Sessions and daily attempts are stored only in Memory;
3.7.7 Failed score submissions cannot be resubmitted;
3.7.8 Shield／invincibility collisions can still create invalid coordinates;
3.7.9 Idempotency retries can incorrectly change 4xx responses into 200;
3.7.10 Basic API access control and security restrictions are incomplete.

3.8 Issue Tracking Fields
...
========================================
3.8.1 Code Review and Recommended Improvements
========================================
...
3.8.2 The README is detailed and includes installation, usage, and customization guidance.
3.8.3 The code structure is clear. The game logic is separated into `preload`, `create`, and `update` functions.
3.8.4 The project has no external asset dependency. Visual elements are generated with graphics code, making the game easy to run and deploy.
3.8.5 Core gameplay functionality is complete, including collision detection, physics, scoring, and the win condition.
3.8.6 Several areas can still be improved to make the code easier to maintain and the game experience more complete.
...
3.8.7 A timeout may continue running after the relevant scene or game state has ended.
3.8.8 Repeated restarts may create multiple pending timers.
3.8.9 Old callbacks may update objects that are no longer active.
3.8.10 This may cause memory leaks or inconsistent game behaviour.

Recommended improvement:

3.8.11 Store each timeout ID.
3.8.12 Clear active timeouts when restarting, leaving, or destroying the scene.
3.8.13 Where possible, use Phaser timer events so that timers follow the Phaser scene lifecycle.
...
3.8.14 Mobile users cannot control the game properly.
3.8.15 The responsive layout does not provide a complete mobile gameplay experience.
3.8.16 The game may work visually inside a mobile browser or WebView but remain unplayable.
...
3.8.18 Move right.
3.8.19 Jump.
3.8.20 Pause, where applicable.

The touch buttons should:

3.8.21 Support `pointerdown` and `pointerup` events.
3.8.22 Prevent accidental page scrolling while playing.
3.8.23 Be large enough for mobile use.
3.8.24 Respect device safe areas.
3.8.25 Work consistently in both mobile browsers and App WebViews.
...
3.8.26 Players cannot temporarily stop the game.
3.8.27 Players may need to reload the page after losing or winning.
3.8.28 The user experience is less complete on both desktop and mobile.
3.8.29 Timers and physics may continue running when the player expects the game to be paused.
...
3.8.30 A pause button.
3.8.31 A resume button.
3.8.32 A restart button.
3.8.33 A complete reset of player position, score, collectibles, timers, and win state.
3.8.34 Input blocking while the game is paused, won, or over.

3.8.4 Recommended Implementation Order

3.8.35 Replace important magic numbers with named constants.
3.8.36 Add cleanup for all `setTimeout` calls and Phaser timer events.
3.8.37 Add mobile touch controls.
3.8.38 Add explicit game state management.
3.8.39 Add pause, resume, and restart functions.
3.8.40 Test restart behaviour to confirm that timers and event listeners are not duplicated.
3.8.41 Test the game on desktop browsers, mobile browsers, and App WebViews.

3.8.5 Suggested Acceptance Criteria

3.8.42 All important gameplay values are defined as named constants.
3.8.43 Restarting the game does not create duplicate timers or event listeners.
3.8.44 No timer callback runs after its scene has been shut down.
3.8.45 The game can be fully controlled by keyboard.
3.8.46 The game can be fully controlled by touch on mobile devices.
3.8.47 Pause stops player movement, physics, score updates, and timed events.
3.8.48 Resume restores the game without resetting progress.
3.8.49 Restart returns the game to a clean initial state.
3.8.50 Winning or losing displays a clear restart option.
3.8.51 The game works without external image assets.
