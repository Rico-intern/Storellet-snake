...
This README explains how the Storellet Jai Snake Game works.

It is written for both technical and non-technical readers. You do not need programming knowledge to understand the main sections.

The document covers:

1. The game rules and how the game behaves.
2. How the game stores player and game-session data in Supabase.
3. What each project file is used for.
4. How to set up and test the project.
5. What is already completed and what still needs to be done before production launch.

## Quick Explanation of the System

The project contains one HTML game file and one Supabase database setup file.

- The HTML file displays and runs the game.
- Supabase identifies the player, checks daily game attempts, stores player data, and stores each game result.
- `package.json` describes the project and provides simple checking commands.
- `package-lock.json` records the exact project package setup so that different computers use the same environment.
- `README.md` explains the complete project.

No separate Node.js API server is required in this version.
...
1.18.4 Before the game starts, the HTML asks Supabase to create a new Game Session.
1.18.5 Supabase checks the player's daily attempt count and points balance.
1.18.6 The game starts only when Supabase confirms that the player is allowed to play.
1.18.7 If the attempt requires points, Supabase deducts the points when the Game Session is created.
1.18.8 The current starter version stores the game-related points balance in the `players` table.
1.18.9 Before production launch, the team must confirm whether Storellet's existing membership points system or Supabase is the official source of truth.

1.19 Score Submission and Reward Issuance
1.19.1 When a game starts, Supabase creates a unique Session ID.
1.19.2 After the game ends, the HTML sends the final score, game duration, and end reason to Supabase.
1.19.3 The same Session accepts only one valid final result.
1.19.4 A Supabase database function determines the reward level from the final score.
1.19.5 Each completed game can receive 1 matching reward result.
1.19.6 The current starter version records the reward code and reward status in the `game_sessions` table.
1.19.7 The actual coupon is not yet issued by Supabase.
1.19.8 Before production launch, `reward_code` must be connected to Storellet's real coupon issuance service.
1.19.9 The intended coupon validity period is 7 days after issuance.
1.19.10 The intended coupon delivery time is within 2 business days.
1.19.11 The intended coupon cannot be used together with other offers.

1.20 Production System Integration Controls
1.20.1 Player identity is verified through Supabase Auth and an access token.
1.20.2 The player's daily attempt count and game-related points balance are stored in Supabase.
1.20.3 Every Game Session has a unique Session ID.
1.20.4 The same Session can submit only one valid final result.
1.20.5 The same Session must not deduct points more than once.
1.20.6 Supabase checks the Session status before pause, resume, or finish actions.
1.20.7 The database validates basic score and game-duration ranges.
1.20.8 Production coupon issuance is not yet connected.
1.20.9 Production still requires stronger anti-cheat checks, operational logs, retry handling, and reconciliation.
1.20.10 High-value point deduction and coupon issuance should be handled by a trusted backend or Supabase Edge Function.
...
- The game screen and retro visual design.
- The snake, food, monsters, obstacles, and treasure chest logic.
- Mobile D-pad controls and keyboard controls.
- Pause, resume, restart, and background handling.
- Supabase connection settings.
- Calls to the Supabase database functions.

A user opens this file in a browser or inside the Storellet App WebView to play the game.
...
- The database functions used to start, pause, resume, and finish a game.
- Row Level Security rules that restrict each player to their own data.

This file is normally copied into the Supabase SQL Editor and run once when setting up the project.

2.1.3 `package.json`

This is the main project information file for Node.js and npm.

In simple terms, it tells a computer:

- The project name and version.
- That the project is private and should not be published as an npm package.
- Which Node.js version should be used.
- Which checking commands are available.

This project does not need npm packages to run the game. The file is mainly used for project organisation and validation commands.

2.1.4 `package-lock.json`

This file records the exact package setup linked to `package.json`.

Even though this project currently has no external runtime packages, the lock file is still useful because it:

- Keeps the project setup consistent across different computers.
- Records the exact project version and npm lock format.
- Prevents unexpected environment changes if packages are added later.
- Supports repeatable setup in development, UAT, and CI environments.

This file is normally generated and updated by npm. It should not be edited manually.

2.1.5 `README.md`

This is the instruction and reference document you are reading.
...
- The file structure.
- The Supabase setup.
- The database tables and functions.
- The installation and testing steps.
- The current limitations and production requirements.

2.1.6 Removed Files

The following files were used by the older Mock API version and are no longer part of the current project:

- `server.js`
- `openapi.yaml`
- `Storellet_Snake_API.postman_collection.json`

They were replaced by Supabase database tables and RPC functions.

2.2 Overall Architecture

The following flow describes what happens when a player uses the game:

```text
Storellet App / Mobile WebView / Browser
                |
                | Opens the HTML game
                | Provides a Supabase access token
                v
...
Supabase
                |
                |-- Supabase Auth identifies the player
                |-- players stores one record per player
                |-- game_sessions stores every game attempt
                `-- Database functions control start, pause, resume, and finish
```

Plain-language explanation:

1. The player opens the game.
2. The App gives the game a valid Supabase access token.
3. Supabase uses the token to identify the player.
4. When the player starts a game, Supabase checks the daily attempt count and points balance.
5. Supabase creates a Game Session.
6. During the game, pause and resume actions update the same Session.
7. When the game ends, the score and result are stored in Supabase.
8. The player's total games, total score, and best score are updated.

No separate Node.js API server is required for this starter version.

2.3 Supabase Tables

2.3.1 `players`

This table stores one summary record for each authenticated player.

Think of it as the player's game profile. It does not store every individual game. Instead, it stores totals and the current daily status.
...
This table stores one record for every game attempt.

Think of it as the game history. Each time a player starts a new game, Supabase creates a new row in this table.
...
The database setup creates four RPC functions.

An RPC function is a controlled database action. The HTML calls the function, and Supabase performs the required checks and updates inside the database.

The functions use `auth.uid()` to identify the logged-in player. This prevents the HTML from simply choosing another player's ID.
...
Supabase Auth is the login and identity system used by the database.

The access token proves which player is making the request. The HTML sends this token to Supabase whenever it starts, pauses, resumes, or finishes a Game Session.
...
Before the HTML can connect to Supabase, the Storellet App WebView must provide several settings.

The App should inject:

2.6.1 `window.STORELLET_SUPABASE_URL`

The web address of the Supabase project.

2.6.2 `window.STORELLET_SUPABASE_ANON_KEY`

The public Supabase anon key. This key identifies the Supabase project. It is not the player's login token.

2.6.3 `window.STORELLET_SUPABASE_ACCESS_TOKEN`

The logged-in player's Supabase access token. Supabase uses this token to identify the player.

2.6.4 `window.STORELLET_SUPABASE_ENABLED = true`

Turns on the Supabase connection.

2.6.5 `window.STORELLET_SUPABASE_REQUIRED = true`

Recommended for production. If Supabase cannot be reached, the game must not continue in offline mode.

2.6.6 `window.STORELLET_GAME_PLATFORM`

Identifies the client, for example `ios`, `android`, or `webview`.

2.6.7 `window.STORELLET_GAME_APP_VERSION`

Records the Storellet App version used for the Game Session.

For local UAT, the Supabase URL and anon key may also be passed through query parameters.

Do not place the player's access token in a public URL. The App should inject it directly into the WebView.
...
2.7.2 Set Up the Database

1. Open the Supabase project.
2. Open the SQL Editor.
3. Open the local `supabase.sql` file.
4. Copy all SQL content.
5. Paste it into the Supabase SQL Editor.
6. Run the SQL.
7. Confirm that the `players` and `game_sessions` tables appear in the Table Editor.

2.7.3 Create a Test Player

1. Enable the required Supabase authentication method.
2. Create or sign in with a test user.
3. Obtain a valid access token for that user.

2.7.4 Add Test Points

The first game each day is free.

To test the second and third games:

1. Open the `players` table.
2. Find the test player's row.
3. Enter a suitable number in `points_balance`, for example `100`.
4. Save the row.

2.7.5 Configure the HTML

Provide the following values before opening the game:

```javascript
window.STORELLET_SUPABASE_URL = "https://YOUR_PROJECT.supabase.co";
window.STORELLET_SUPABASE_ANON_KEY = "YOUR_ANON_KEY";
window.STORELLET_SUPABASE_ACCESS_TOKEN = "PLAYER_ACCESS_TOKEN";
window.STORELLET_SUPABASE_ENABLED = true;
window.STORELLET_SUPABASE_REQUIRED = true;
window.STORELLET_GAME_PLATFORM = "webview";
window.STORELLET_GAME_APP_VERSION = "uat";
```

Replace the example values with the real project values.

2.7.6 Open the Game

Open `Storellet_snake_supabase_integrated.html` in a supported browser or inside the Storellet App WebView.

2.7.7 Basic Test Checklist

Confirm that:

1. The first game of the day is free.
2. The second and third games deduct 10 points each.
3. A fourth game cannot start.
4. Pause and resume work.
5. A completed score is stored in `game_sessions`.
6. `best_score`, `total_games`, and `total_score` update in `players`.
7. One Session cannot be finished twice.

2.8 Package Files

2.8.1 Why `package.json` Exists

`package.json` is a standard project file used by Node.js and npm.

In this project, it is used mainly to:

- Give the project a name and version.
- Define the minimum Node.js version.
- Provide checking commands.
- Keep the project ready for future automated testing or CI.

The game itself does not need Node.js to run in the browser.

2.8.2 Why `package-lock.json` Exists

`package-lock.json` helps make the project setup repeatable.

It ensures that:

- The same package setup is used on different computers.
- Future dependencies can be installed consistently.
- CI, UAT, and production checks do not unexpectedly use different package versions.

The file should be kept together with `package.json`.
...
This checks the package setup and creates the local npm environment.

```bash
npm run check:json
```

This checks that `package.json` contains valid JSON and can be read correctly.

Because the current project has no external runtime dependencies, `npm install` does not install the game engine or Supabase. The game continues to run as a standalone HTML file.
...
The browser or App WebView sends information directly to Supabase using the player's access token.

This is acceptable for a starter or UAT version.

However, a player can potentially inspect or modify browser-side requests. For that reason, valuable actions such as official Storellet point deduction and coupon issuance should not trust the browser alone.

Before production launch, these actions should be checked and completed by a trusted backend or Supabase Edge Function.
